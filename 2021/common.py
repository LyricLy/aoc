...
class Grid(Generic[T]):
    ...
    def __setitem__(self, p: Point, x: T):
        if p not in self:
            raise ValueError(f"assigning to out of bounds point {p}")
    ...
    def take(self, ps: Iterable[Point]) -> list[T]:
        ...
        for p in ps:
            x = self[p]
            if x is None:
                raise ValueError(f"out of bounds point {p} in take sequence")
            l.append(x)
        return l
    ...
    def parse(cls, s: str, convert: Callable[[str], T], split: Callable[[str], Iterable[str]] = list) -> Grid[T]:
        return cls.from_list(map(convert, split(r)) for r in s.strip().splitlines())
